...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-r", "--refseq", dest="refseq", help="reference sequence, supplied as a fasta file")
    ...
    parser.add_argument(
        "--output", dest="outputfile", default=None, help="specify an outputfile name")
    ...
    print_('Loaded alignment of %i sequences, %i positions.' %
    ...
    print_("Aligment size after removing sequences with non-standard amino acids: %i" %
           (len(sequences_full)))
    ...
    sequences_full = alg_out
    print_("Alignment size post-trimming: %i positions" % len(sequences_full[0]))
    ...
    if options.i_ref is None:
        if options.pdbid is not None:
            try:
                ...
                if options.species is not None:
                    try:
                        ...
                        Options_ref = i_ref
                        print_("reference sequence index is: %i" % (i_ref))
            ...
            print_("No reference sequence given, chose as default (%i): %s" %
                   (i_ref, headers_full[i_ref]))
            sequences = sequences_full
            ats = range(len(sequences[0]))
    else:
        print_("using provided reference index %i" % (i_ref))
        print_(headers_full[i_ref])
    ...
    print_("Conducting sequence and position filtering: alignment size is %i seqs, %i pos" %
           (len(sequences), len(sequences[0])))
    if options.pdbid is not None:
        print_("ATS and distmat size - ATS: %i, distmat: %i x %i"
               % (len(ats), len(dist_pdb), len(dist_pdb[0])))
    else:
        print_("ATS should also have %i positions - ATS: %i" % (len(sequences[0]), len(ats)))
    ...
    print_("After filtering: alignment size is %i seqs, %i effective seqs, %i pos" %
    ...
    print_("Number of sequences: M = %i" % (Nseq))
    print_("Number of effective sequences: M' = %i" % (effseqs))
    print_("Number of alignment positions: L = %i" % (Npos))

    if options.pdbid is not None:
        print_("Number of positions in the ats: %i" % (len(ats)))
        structPos = [i for (i, k) in enumerate(ats) if k != '-']
        print_("Number of structure positions mapped: %i" % (len(structPos)))
        print_("Size of the distance matrix: %i x %i" % (len(distmat), len(distmat[0])))
    ...
    print_("Opening database file " + "Outputs/" + fn_noext)
    db = {}
    db['sequence'] = D
