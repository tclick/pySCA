...
if __name__ =='__main__':
        ...
        print_('Loaded alignment of %i sequences, %i positions.' % (len(headers_full), len(sequences_full[0])))
        ...
        print_("Aligment size after removing sequences with non-standard amino acids: %i" % (len(sequences_full)))
        ...
        if options.i_ref is None:
                if options.pdbid is not None:
                        ...
                        print_("No reference sequence given, chose as default (%i): %s" % (i_ref, headers_full[i_ref]))
        ...
        print_("Conducting sequence and position filtering: alignment size is %i seqs, %i pos" % (len(sequences), len(sequences[0])))   
        if options.pdbid is not None:
        ...
        print_("After filtering: alignment size is %i seqs, %i effective seqs, %i pos" % (len(alg1), effseqsprelimit, len(alg1[0])))        
