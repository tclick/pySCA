...
def readAlg(filename):
    ...
    sequences = list()
    notfirst = 0
    with open(filename, 'r') as alnfile:
        for line in alnfile:
            if line[0] == '>':
                if notfirst > 0:
...
def basicICA(x, r, Niter):
    ...
    **Example:**
    ...
    for _ in range(Niter):
        ...
        u = w.dot(x)
        w += r * \
            (M * np.eye(L) + (1 - 2 * (1. / (1 + np.exp(-u)))).dot(u.T)).dot(w)
        delta = (w - w_old).ravel()
...
def rotICA(V, kmax=6, learnrate=.0001, iterations=10000):
    ...
    for n in range(kmax):
        ...
        Vica[:, n] = np.sign(
            Vica[imax, n]) * Vica[:, n] / np.linalg.norm(Vica[:, n])
    return Vica, W
...
def projUica(msa_ann, msa_num, seqw, kica=6):
    ...
    for n in range(kica):
        ...
        Ui1[:, n] = np.sign(Ui0[imax, n]) * Ui1[:, n] / \
            np.linalg.norm(Ui0[:, n])
        Ui0[:, n] = np.sign(Ui0[imax, n]) * Ui0[:, n] / \
            np.linalg.norm(Ui0[:, n])
    return Ui1, Ui0
...
def numConnected(Vp, k, distmat, eps_list=np.arange(.5, 0, -.01), dcontact=5):
    ...
    **Example:**
    ...
    for eps in eps_list:
        ...
        group = [i for i in range(Npos) if (
            Vp[i, k] > eps and Vp[i, k] > Vothers[i])]
...
def icList(Vpica, kpos, Csca, p_cut=0.95):
    ...
    for k in range(kpos):
        ...
        iqr = scoreatpercentile(
            Vpica[:, k], 75) - scoreatpercentile(Vpica[:, k], 25)
